...
DEST_KMSKEYID = os.getenv('DEST_KMSKEYID', 'None').strip()
...
def lambda_handler(event, context):
    ...
    if pending_snapshots_copy > 0:
        log_message = 'Could not re-encrypt all snapshots. Pending: %s' % pending_snapshots_copy
        logger.error(log_message)
        raise SnapshotToolException(log_message)
